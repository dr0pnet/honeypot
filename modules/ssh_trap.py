...
host = config.get("ssh_host", "0.0.0.0")
port = config.get("ssh_port", 2222)

SSH_LOG_DIR = os.path.join(PROJECT_ROOT, "logs", "ssh_trap")
os.makedirs(SSH_LOG_DIR, exist_ok=True)
SSH_LOG_FILE = os.path.join(SSH_LOG_DIR, "ssh_trap.log")
...
def handle_termination(signum, frame):
    print("[INFO] Shutting down SSH trap...")
    if server_socket:
        server_socket.close()
...
def handle_client(conn, addr):
    ...
    with open(SSH_LOG_FILE, "a") as log:
        log.write(f"[{timestamp}] SSH connection from {attacker_ip}\n")

    try:
        conn.send(b'SSH-2.0-OpenSSH_7.9\r\n')
    except Exception as e:
        print(f"[DEBUG] Failed to send SSH banner: {e}")
    finally:
        conn.close()

    # Run passive intel modules
    try:
        ...
        for script in [ip_geo_path, reverse_dns_path, passive_scan_path]:
            p = subprocess.Popen([PYTHON_CMD, script, attacker_ip, SSH_LOG_DIR], cwd=PROJECT_ROOT)
            child_processes.append(p)

        trigger_alert(f"SSH trap hit from {attacker_ip}")
    except Exception as e:
        print(f"[Intel ERROR] SSH intel module failed: {e}")
...
def run():
    ...
    try:
        ...
        print(f"[dr0pnet] SSH trap listening on {host}:{port}...")
        while True:
